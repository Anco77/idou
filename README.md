...
- **补货/消耗**：单个或批量操作，库存和变更日志原子写入；批量库存不足时整批取消并逐项显示缺口
- **余量排序**：一键按库存数量排序，扁平列表展示
- **四级预警**：< 400 紧缺 / 400-700 较少 / 700-1000 适中 / > 1000 充足，颜色+进度条直观提示
- **一键初始化**：将所有色号库存重置为指定数量
- **色号详情**：查看单个色号的操作历史
- **库存流水**：按类型、色号和时间筛选并分页加载；图纸扣库可跳转原图纸，撤销使用一次性冲正流水保留审计历史
- **自动更新**：通过 GitHub Releases 检查更新并下载安装
- **搜索过滤**：按 MARD 色号、名称或内部编号实时搜索
- **低量预警**：阈值可配置，修改后首页、库存列表和色号详情即时统一
