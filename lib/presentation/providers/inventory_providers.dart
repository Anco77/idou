...
class InventoryState {
  ...
  List<InventoryWithColor> get filteredItems {
    ...
    if (searchQuery.isNotEmpty) {
      ...
      result = result.where((item) => item.matches(query)).toList();
    }
...
final inventoryStateProvider =
  ...
  final notifier = InventoryStateNotifier(service, ref);
  ref.listen<int>(
    userSettingsProvider.select((settings) => settings.lowStockThreshold),
    (_, threshold) => notifier.updateLowStockThreshold(threshold),
  );
  return notifier;
});
...
class InventoryStateNotifier extends StateNotifier<InventoryState> {
  ...
  InventoryStateNotifier(this._service, this._ref)
  ...
  Future<void> loadInventory() async {
    final threshold = _ref.read(userSettingsProvider).lowStockThreshold;
    ...
    try {
      ...
      state = state.copyWith(
        items: items,
        lowStockItems:
            items.where((item) => item.isLowStockAt(threshold)).toList(),
        isLoading: false,
        clearError: true,
      );
    } catch (e) {
  ...
  void updateLowStockThreshold(int threshold) {
    state = state.copyWith(
      lowStockThreshold: threshold,
      lowStockItems:
          state.items.where((item) => item.isLowStockAt(threshold)).toList(),
    );
  }
  ...
  Future<BatchAdjustResult> batchAdjust(
    ...
    try {
      final result = await _service.batchAdjust(quantities, restock: restock);
...
/// 获取指定色号的操作日志
final colorLogsProvider = FutureProvider.autoDispose
    .family<List<InventoryLogItem>, int>((ref, colorId) {
  final service = ref.watch(inventoryServiceProvider);
  return service.getLogsForColor(colorId, limit: 50);
});

/// 全局操作日志（按时间倒序，可筛选类型）
final operationLogsProvider = FutureProvider.autoDispose
    .family<List<OperationLogItem>, String?>((ref, filterType) {
  final service = ref.watch(inventoryServiceProvider);
